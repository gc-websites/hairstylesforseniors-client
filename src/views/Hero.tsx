  ...
  return (
    <section className="container section__padding pt-8 md:pt-12">
      ...
      <div className="grid gap-6 md:grid-cols-[1.7fr_1fr] md:gap-8 md:items-stretch">
        {/* Lead story — large image, full intro, byline + CTA. The image is
            flex-1 so it absorbs any height the (stretched) column has beyond the
            text, which keeps the card full with no dead space below the copy. */}
        <Link
        ...
        >
          <div className="relative w-full flex-1 min-h-[16rem] sm:min-h-[20rem] overflow-hidden">
            <img
              src={lead.image.url}
              alt={lead.title}
              loading="eager"
            ...
            {lead.category_3?.name && (
              <span className="absolute bottom-4 left-4 inline-flex max-w-[70%] items-center truncate rounded-full bg-main3 px-3 py-1 font-poppins text-xs font-semibold uppercase tracking-wide text-white shadow-sm">
                {lead.category_3.name}
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4 p-6 md:p-8">
            <h2 className="section__title text-2xl leading-snug transition-colors group-hover:text-main3 dark:group-hover:text-main md:text-3xl">
              {lead.title}
            </h2>
            <RenderDescription
        ...
        {/* Two secondary stories — same card grammar, image flex-1 so the pair
            fills the column height beside the lead with no ragged gaps. */}
        <div className="flex flex-col gap-6 md:gap-8">
          {sidePosts.map(post => (
            ...
            >
              <div className="relative w-full flex-1 min-h-[11rem] overflow-hidden">
                <img
                  src={post.image.url}
                  alt={post.title}
                  loading="lazy"
                ...
                {post.category_3?.name && (
                  <span className="absolute bottom-3 left-3 inline-flex max-w-[80%] items-center truncate rounded-full bg-main3 px-2.5 py-1 font-poppins text-[11px] font-semibold uppercase tracking-wide text-white shadow-sm">
                    {post.category_3.name}
                  </span>
                )}
              </div>
